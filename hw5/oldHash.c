...
void putInChain(Chain oldChain, Triple newTrip);
...
void insertH(Hash h, char *position, char *from, int nSteps)
{
    ...
    newTrip->position = strdup(position);
    if(from != NULL){
        newTrip->from = strdup(from);
    }else{
        char* null = "NULL";
        newTrip->from = strdup(null);
    }
